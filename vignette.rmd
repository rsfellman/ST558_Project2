...
Before creating functions to query specific endpoints of the API, I will create a function to rename certain variables, making them easier to understand. The input for this function is a singular `data.frame`. 
...
```{r}
earthquakes.low <- query.data(min.mag = -1, max.mag = 4, max.gap = 180)
earthquakes.low
```
...
I will also create a pull data for all earthquakes magnitude -1 to 10.
```{r}
...
I am also curious about the distribution of the Hawaii earthquake magnitudes in comparison to all smaller earthquakes. I will create a density plot of the Hawaii data (colored in green) layered over all of the smaller (magnitude < 4) earthquake data to see if they follow a similar distributions. The reason I am using the low magnitude data is because the maximum magnitude for Hawaii earthquakes was 3.53.
```{r}
#create base plot
...
#add a color gradient for the magnitude of the earthquakes
   scale_colour_gradient(low = "lightblue", high = "navy", name = "Magnitude")+
#adjust the coordinates
  coord_cartesian(xlim = c(-127,-110), ylim = c(30,50)) +
#add labels.
...
The interesting thing about this plot is that we can actually see the San Andreas Fault that follows the California Coast. The map used a color gradient for the magnitude of the earthquake, allowing more information to be displayed. Interestingly the highest magnitude earthquake does not appear to be on the San Andreas Fault, but is instead inland in Northern California. This could be of interest for future studies. The majority of the earthquakes seem to be in the 1 to 3 magnitude range.


Since the California map provided a lot of information, I will also plot the earthquake data for the entire globe. The points will once again be colored by magnitude. First I will call data from the API using the `eq.location` function with the latitude and longitude both set to 0 and the maximum radius at its maximum value.
```{r}
#call data from API
...
#add the points of the earthquakes in california
   geom_point(data = eq.nc, aes(x = longitude, y = latitude, colour = magnitude))  +
#add a color gradient for the magnitude of the earthquakes
...
Next I will compare 2 numerical variables `mag` and `sig`. `sig` is a number that signifies the significance of the seismic event. While the calculation of this number does take into account the magnitude, it also includes other factors such as intensity, felt reports, and estimated impact. While I would guess that there would be a positive relationship between the two variables, this graph will help to see the relationship.

```{r}
c<- ggplot(earthquakes.all)
c+ geom_point(aes(x = station.distance, y = magnitude)) +
  labs(title = "Scatterplot of Earthquake Manitudes vs. Significance", x = "Magnitude", y= "Significance")
```
Here we can see the positive relationship between magnitude and significance. The interesting aspect of this plot is the outlier. It appears there is one earthquake with a significance much higher than all of the other. This may be a location that needs more investigation.